...
\tikzstyle{startstop} = [rectangle, rounded corners, minimum width=2cm, minimum height=3cm,node distance = 4 cm,text centered, draw=black, fill=white]
\tikzstyle{arrow} = [thick,->,>=stealth]
\tikzstyle{blocks} = [ rectangle , minimum width=5cm, minimum height=12cm,node distance = 2 cm,text centered, draw=black, fill=none]

\begin{tikzpicture}[node distance=1.6cm]
\node (node0) [startstop] {Grid Generation};
\node (node1) [startstop, right of = node0] {Vehicle Counting};
\node (node2) [startstop, right of = node1] {Patch Centroid};
%\node (node1) [blocks] {1} ;
\node (node3) [startstop, below of = node0] {Grid Generation};
\node (node4) [startstop, right of = node3] {Vehicle Counting};
\node (node5) [startstop, right of = node4] {Lane Change};

\node (node6) [startstop, below of = node3] {Occupancy Array};
\node (node7) [startstop, right of = node6] {Vehicle Removal};
\node (node8) [startstop, right of = node7] {Vehicle Mapping};
%\draw [arrow] (node0) -- (node1);

\begin{comment}
\node (node0) [startstop] {Traffic video};
\node (node1) [startstop, left of = node0] {Vehicle Centroid from Tracking};
\node (node2) [startstop, left of = node1] {Custom ROI for each vehicle};
\node (node3) [startstop, below of = node2] {Detect Edges};
\node (node4) [startstop, right of = node3] {Dilate + Erode};
\node (node5) [startstop, right of = node4] {Develop Square Box};
\node (node6) [startstop, below of = node5] {Reduce to Rectangle};
\node (node7) [startstop, left of = node6] {Extract Vehicle Dimensions};
